...
/// Generates the `into_*` / `as_* `/ `is_*` accessor trio for one variant per
/// tag.
///
/// The three leading literals are the prose, because the same trio is generated
/// for two types that mean different things by it: a [`Value`] variant holds
/// *one* value of the tag, a [`ValueList`] variant holds a `Vec` of them. Left
/// shared, the generated text would be wrong for one of the two.
macro_rules! impl_access_fns {
    (
        subject = $subject: literal,
        condition = $condition: literal,
        ...
        $(paste::paste! {
            #[inline]
            #[doc = concat!(
                "Returns ", $subject, " if ", $condition, " `", stringify!($tag), "`, ",
                "otherwise returns `self` unchanged.\n\n",
                "The owning counterpart of [`as_", stringify!([<$tag:snake>]), "`]",
                "(Self::as_", stringify!([<$tag:snake>]), ")."
            )]
            ...
            #[inline]
            #[doc = concat!(
                "Returns a reference to ", $subject, " if ", $condition, " `",
                stringify!($tag), "`, otherwise `None`.\n\n",
                "To take ownership instead, use [`into_", stringify!([<$tag:snake>]), "`]",
                "(Self::into_", stringify!([<$tag:snake>]), ")."
            )]
            ...
            #[inline]
            #[doc = concat!(
                "Returns whether ", $predicate, " `", stringify!($tag), "`."
            )]
...
impl Value {
    impl_access_fns!(
        subject = "the inner value",
        condition = "the tag is a",
        predicate = "the inner value is a";
        Byte = i8,
        Short = i16,
        Int = i32,
        Long = i64,
...
impl ValueList {
    ...
    /// `&list` in a `for` loop does the same thing.
    pub fn iter(&self) -> ValueListIter<'_> {
        ValueListIter {
            list: self,
            front: 0,
            back: self.len(),
...
/// The `into_*`/`as_*`/`is_*` trio, one per element type.
///
/// These are named like [`Value`]'s and generated by the same macro, but they
/// answer a different question and return a different thing: a `ValueList`
/// variant holds *every* element, so where `Value::as_byte` gives an
/// `Option<&i8>` — the one byte this value is — `ValueList::as_byte` gives an
/// `Option<&Vec<i8>>`: all the bytes this list holds. Likewise
/// `Value::as_list` is an `Option<&ValueList>` (the payload of one list) while
/// `ValueList::as_list` is an `Option<&Vec<ValueList>>` (a list *of* lists).
/// Reach for [`Self::get`] or [`Self::iter`] to get individual elements back as
/// `Value`s.
impl ValueList {
    impl_access_fns!(
        subject = "the elements",
        condition = "this is a list of",
        predicate = "this is a list of";
        Byte = Vec<i8>,
        Short = Vec<i16>,
        Int = Vec<i32>,
        Long = Vec<i64>,
...
/// Iterates a [`ValueList`] by reference, cloning each element into a [`Value`].
///
/// The elements are stored unboxed — a `Vec<i8>`, not a `Vec<Value>` — so there
/// is no `Value` to hand out a reference to and one has to be built; see
/// [`ValueList::get`]. Created by [`ValueList::iter`], and by `&list` in a
/// `for` loop.
#[derive(Debug, Clone)]
pub struct ValueListIter<'a> {
    ...
    front: usize,
    /// One past the index of the next element from the back; `front == back`
    /// means exhausted, from either end.
    back: usize,
}

impl Iterator for ValueListIter<'_> {
    type Item = Value;
    ...
    fn next(&mut self) -> Option<Value> {
        ...
        // `front < back <= len` here, and each variant's storage holds exactly
        // `len` elements, so `get` cannot answer `None`. If it ever did,
        // panicking is the only honest answer: substituting a value would
        // quietly corrupt whatever document is being walked.
        let value = self.list.get(self.front).expect("index < len");
        self.front += 1;
        Some(value)
    }
...
impl DoubleEndedIterator for ValueListIter<'_> {
    fn next_back(&mut self) -> Option<Value> {
        if self.front == self.back {
            return None;
        }
        self.back -= 1;
        Some(self.list.get(self.back).expect("index < len"))
    }
}

impl ExactSizeIterator for ValueListIter<'_> {}

impl FusedIterator for ValueListIter<'_> {}

/// `for value in &list` — the by-reference counterpart of the owning
/// [`IntoIterator`] below, delegating to [`ValueList::iter`].
impl<'a> IntoIterator for &'a ValueList {
    type Item = Value;
    type IntoIter = ValueListIter<'a>;

    fn into_iter(self) -> ValueListIter<'a> {
        self.iter()
    }
}
...
impl DoubleEndedIterator for ValueListIntoIter {
    fn next_back(&mut self) -> Option<Value> {
        match self {
            Self::End => None,
            Self::Byte(it) => it.next_back().map(Value::Byte),
            Self::Short(it) => it.next_back().map(Value::Short),
            Self::Int(it) => it.next_back().map(Value::Int),
            Self::Long(it) => it.next_back().map(Value::Long),
            Self::Float(it) => it.next_back().map(Value::Float),
            Self::Double(it) => it.next_back().map(Value::Double),
            Self::ByteArray(it) => it.next_back().map(Value::ByteArray),
            Self::String(it) => it.next_back().map(Value::String),
            Self::List(it) => it.next_back().map(Value::List),
            Self::Compound(it) => it.next_back().map(Value::Compound),
            Self::IntArray(it) => it.next_back().map(Value::IntArray),
            Self::LongArray(it) => it.next_back().map(Value::LongArray),
        }
    }
}
...
// Every arm is either `std::vec::IntoIter`, which is fused, or `End`, which
// always answers `None`.
impl FusedIterator for ValueListIntoIter {}
...
/// Total equality, element by element, with [`Value`]'s float rules: every
/// `NaN` equals every other `NaN`, and `-0.0 == 0.0`.
///
